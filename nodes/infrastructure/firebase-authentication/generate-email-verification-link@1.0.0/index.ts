...
export default class GenerateEmailVerificationLink extends BlueprintNode {
    auth!: Auth;
    ...
    async run(ctx: BlueprintContext): Promise<ResponseContext> {
        ...
        try {
            ...
            email = opts?.inputs?.email || ctx.response.data;

            this.initFirebaseAdmin(firebaseConfig);
            app = admin.app(firebaseConfig?.client_email);
            this.auth = app.auth();

            response.data = await this.auth.generateEmailVerificationLink(email);
        } catch (error: any) {
