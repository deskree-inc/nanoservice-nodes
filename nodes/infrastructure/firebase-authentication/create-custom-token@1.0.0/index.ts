...
export default class CreateCustomToken extends BlueprintNode {
    auth!: Auth;
    ...
    async run(ctx: BlueprintContext): Promise<ResponseContext> {
        ...
        try {
            ...
            uid = opts?.inputs?.uid || ctx.response.data;

            this.initFirebaseAdmin(firebaseConfig);
            app = admin.app(firebaseConfig?.client_email);
            this.auth = app.auth();

            response.data = await this.auth.createCustomToken(uid);
        } catch (error: any) {
            response.error = this.setError(error);
            response.error.setCode(400);
